...
const hit: ScanResult = {
  ...
  margin: 30,
  candidates: [{
    card_number: 'OP01-001', language: 'en', name: 'Monkey.D.Luffy', distance: 16,
    printings: [{ card_id: 'OP01-001', pack_code: 'OP-01', pack_name: 'ROMANCE DAWN', rarity: 'Leader' } as never],
    ambiguous_printing: false, card,
  }],
  message: null,
}
...
describe('scanner', () => {
  ...
  it('warns when the scanned card is already held, and offers both answers', async () => {
    ...
    expect(await screen.findByText(/Déjà dans ta collection/)).toHaveTextContent('×2')
    expect(screen.getByRole('button', { name: 'Ajouter un exemplaire' })).toBeInTheDocument()
    ...
    expect(screen.queryByRole('button', { name: 'Ranger dans la collection' })).toBeNull()
  })
  ...
  it('adds nothing when the duplicate is passed over', async () => {
    ...
    await waitFor(() => expect(screen.queryByText(/Déjà dans ta collection/)).toBeNull())
  ...
  it('asks plainly for a card that is not held yet', async () => {
    ...
    expect(await screen.findByRole('button', { name: 'Ranger dans la collection' })).toBeInTheDocument()
